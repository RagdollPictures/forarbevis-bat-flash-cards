import type { FlashCard } from "../../../../constants/flashcards/types";

export const sakerhetFlytvast: FlashCard[] = [
  {
    id: "sakerhet_flytvast-1",
    question:
      "Vad ska man precis få in under midjebandet på en flytväst som är lagom spänd?",
    questionQuiz:
      "Hur kontrollerar man att en flytväst sitter lagom hårt runt midjan?",
    answer:
      "Knytnäven på den som använder flytvästen.",
    options: [
      "Två fingrar.",
      "En knytnäve.",
      "En hel hand.",
      "Ingenting ska få plats.",
    ],
    correctOptionIndex: 1,
    textTitle: "Rätt spänd flytväst",
textInfo: "En flytväst ska sitta lagom hårt. Man ska precis kunna få in sin knutna hand mellan kroppen och midjebandet. Sitter den för löst kan den glida av, och sitter den för hårt blir den obekväm."
  },
  {
    id: "sakerhet_flytvast-2",
    question:
      "Vilken typ av flytväst ska barn ha som befinner sig nära vatten?",
    questionQuiz:
      "Vilken typ av flytutrustning rekommenderas för barn nära vatten?",
    answer:
      "Räddningsväst.",
    options: [
      "Seglarväst.",
      "Uppblåsbar väst.",
      "Räddningsväst.",
      "Ingen väst behövs om vuxen är nära.",
    ],
    correctOptionIndex: 2,
    textTitle: "Flytväst för barn",
textInfo: "Barn som befinner sig nära vatten ska använda en räddningsväst med krage. Den vänder barnet så att ansiktet hamnar uppåt i vattnet och minskar risken för drunkning."
  },
  {
    id: "sakerhet_flytvast-3",
    question:
      "Vad ska en bra flytväst minst vara utrustad med?",
    questionQuiz:
      "Vilka säkerhetsdetaljer bör en flytväst minst ha?",
    answer:
      "Grenband, reflexer och visselpipa.",
    options: [
      "Fickor och dragkedja.",
      "Grenband, reflexer och visselpipa.",
      "GPS och lampa.",
      "Endast reflexer.",
    ],
    correctOptionIndex: 1,
    textTitle: "Viktig utrustning",
textInfo: "En bra flytväst bör minst vara utrustad med grenband, reflexer och visselpipa. Grenbandet håller västen på plats, reflexer gör att man syns bättre och visselpipan gör att man kan kalla på hjälp."
  },
  {
    id: "sakerhet_flytvast-4",
    question:
      "Hur ofta ska en uppblåsbar flytväst kontrolleras?",
    questionQuiz:
      "Hur ofta bör en uppblåsbar flytväst ses över?",
    answer:
      "Minst en gång per år.",
    options: [
      "Varje vecka.",
      "Minst en gång per år.",
      "Vart femte år.",
      "Endast efter användning.",
    ],
    correctOptionIndex: 1,
    textTitle: "Kontroll av uppblåsbar väst",
textInfo: "En uppblåsbar flytväst ska kontrolleras minst en gång per år. Detta säkerställer att utlösaren, gaspatronen och själva västen fungerar som de ska."
  },
  {
    id: "sakerhet_flytvast-5",
    question:
      "Kan man kontrollera sin uppblåsbara flytväst själv?",
    questionQuiz:
      "Är det möjligt att själv göra en kontroll av sin uppblåsbara flytväst?",
    answer:
      "Ja, genom att följa en enkel instruktion.",
    options: [
      "Nej, endast verkstad får göra det.",
      "Ja, enligt tillverkarens instruktion.",
      "Endast vart tredje år.",
      "Nej, den är självkorrigerande.",
    ],
    correctOptionIndex: 1,
    textTitle: "Egen kontroll",
textInfo: "Man kan själv kontrollera en uppblåsbar flytväst efter att ha lärt sig hur det görs. Det är en enkel men viktig åtgärd för att säkerställa att västen fungerar vid en nödsituation."
  },
  {
    id: "sakerhet_flytvast-6",
    question:
      "Vad är risken med en uppblåsbar flytväst som inte är kontrollerad?",
    questionQuiz:
      "Vad kan hända om en uppblåsbar flytväst inte är regelbundet kontrollerad?",
    answer:
      "Att den inte blåser upp sig vid fall i vattnet.",
    options: [
      "Att den blir för hård.",
      "Att den tappar färg.",
      "Att den inte blåser upp sig vid behov.",
      "Att den flyter för mycket.",
    ],
    correctOptionIndex: 2,
    textTitle: "Risk med okontrollerad väst",
textInfo: "En uppblåsbar flytväst som inte är kontrollerad kan sluta fungera och inte blåsa upp sig i vattnet. Då fungerar den inte som skydd utan blir istället en extra belastning."
  },
];
