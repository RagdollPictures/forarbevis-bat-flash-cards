import type { FlashCard } from "../../../../constants/flashcards/types";

export const miljoMiljovett: FlashCard[] = [
  {
    id: "miljo_miljovett-1",
    question:
      "Får man hälla ut olja i vattnet om man är mer än 12 nautiska mil ifrån land?",
    questionQuiz:
      "Är det tillåtet att släppa ut olja i havet om du befinner dig långt från land?",
    answer: "Nej, aldrig!",
    options: [
      "Ja, om du är mer än 12 nautiska mil från land.",
      "Ja, om det är små mängder.",
      "Nej, det är alltid förbjudet.",
      "Endast i internationellt vatten.",
    ],
    correctOptionIndex: 2,
   textTitle: "Oljeutsläpp",
textInfo: "Det är alltid förbjudet att släppa ut olja i vattnet, oavsett avstånd från land. Olja och bränslen skadar både djurliv och miljö och kan spridas över stora områden på vattenytan. All spillolja ska istället tas om hand och lämnas till en miljöstation."
  },
  {
    id: "miljo_miljovett-2",
    question:
      "I vilket skick ska man lämna en plats i naturen då man åker därifrån?",
    questionQuiz:
      "Hur bör du lämna en naturplats när du avslutar ditt besök?",
    answer:
      "Helst finare och renare än den var när man kom dit, men minst i samma skick.",
    options: [
      "I samma eller bättre skick än när du kom.",
      "Det räcker att ta med sitt eget skräp.",
      "Lite påverkan är acceptabel.",
      "Man ansvarar bara för sin egen båt.",
    ],
    correctOptionIndex: 0,
    textTitle: "Lämna platsen",
textInfo: "En plats i naturen ska lämnas i minst samma skick som när man kom, men helst i bättre skick. Det innebär att man tar med sig allt skräp och gärna plockar upp sådant som andra lämnat efter sig. På så sätt bidrar man till en renare och mer hållbar miljö."
  },
  {
    id: "miljo_miljovett-3",
    question:
      "Nämn tre saker från en båt som kan störa djur och natur förutom skräp.",
    questionQuiz:
      "Vilka faktorer från en båt kan påverka djur och natur negativt, förutom nedskräpning?",
    answer:
      "Svall, buller och avgaser.",
    options: [
      "Fendrar, flaggor och lanternor.",
      "Svall, buller och avgaser.",
      "Navigationsljus och ankare.",
      "Räcken, rep och kapell.",
    ],
    correctOptionIndex: 1,
    textTitle: "Störningar från båt",
textInfo: "Svall, buller och avgaser från båtar kan störa både djur och människor. Höga svallvågor kan skada stränder och båtar, buller kan skrämma djurliv och avgaser påverkar miljön negativt. Att köra lugnt och visa hänsyn är därför viktigt."
  },
  {
    id: "miljo_miljovett-4",
    question:
      "Vad ska man tänka på då man kör sin båt på grunt vatten?",
    questionQuiz:
      "Hur bör du framföra båten vid färd över grunt vatten?",
    answer:
      "Att köra långsamt och försiktigt så att båten, sjöbotten och propellern inte skadas.",
    options: [
      "Hålla hög fart för att minska bottenkontakt.",
      "Köra långsamt och försiktigt.",
      "Stänga av motorn helt.",
      "Styra i sicksack för bättre kontroll.",
    ],
    correctOptionIndex: 1,
    textTitle: "Köra i grunt vatten",
textInfo: "I grunt vatten ska man köra långsamt och försiktigt eftersom botten ofta är känslig. Propellern och båtens rörelser kan skada växt- och djurliv, särskilt i områden med till exempel ålgräs. Låg fart minskar både miljöpåverkan och risken för skador på båten."
  },
  {
    id: "miljo_miljovett-5",
    question:
      "I vilken riktning lyfter normalt en flyende fågel?",
    questionQuiz:
      "När en fågel skräms upp av en båt – åt vilket håll lyfter den oftast?",
    answer: "Mot vinden.",
    options: [
      "Med vinden.",
      "Mot vinden.",
      "Rakt upp utan hänsyn till vind.",
      "Bort från solen.",
    ],
    correctOptionIndex: 1,
    textTitle: "Fåglars flykt",
textInfo: "Flyende fåglar lyfter normalt mot vinden eftersom det ger bättre lyftkraft. Genom att hålla sig på läsidan av fåglar med båten kan man minska risken att störa dem i onödan."
  },
];
