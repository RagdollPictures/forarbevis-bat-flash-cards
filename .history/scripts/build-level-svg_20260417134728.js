...
function addDecoColorSupport(code) {
  return code.replace(
    /<G([^>]*)id="(deco_\d+)"([^>]*)>([\s\S]*?)<\/G>/g,
    (full, beforeId, decoId, afterId, inner) => {
      const updatedInner = inner.replace(/fill="([^"]+)"/g, (fillFull, originalColor) => {
        return `fill={props.decoColors?.${decoId} ?? "${originalColor}"}`;
      });

      return `<G${beforeId}id="${decoId}"${afterId}>${updatedInner}</G>`;
    }
  );
}
...
async function main() {
  ...
  componentCode = addDecoColorSupport(componentCode);
