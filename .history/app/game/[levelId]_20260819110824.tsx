...
import { colorSchemeGui, colorSchemeQuiz } from "../../constants/colors";
...
export default function QuizMenuScreen() {
  ...
  const [levelAreaHeight, setLevelAreaHeight] = useState(0);
...
const backgroundHeight = foregroundHeight * backgroundParallaxFactor;
  ...
  const bgTranslateY = scrollY.interpolate({
    inputRange: [0, maxForegroundScroll],
    outputRange: [0, -maxBackgroundTravel],
    extrapolate: "clamp",
  });
      ...
      >
        ...
        >
          {bonusQuizzes.map((quiz) => {
            ...
            const BonusIcon =
              isUnlocked ? iconSet?.on : iconSet?.off ?? iconSet?.on;
