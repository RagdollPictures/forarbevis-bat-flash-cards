import type { FlashCard } from "../../types";

export const flytandeSjomarken: FlashCard[] = [
  {
    id: "flytande_sjomarken-1",
    question: "Vilka två färger används på kardinalmärken?",
    questionQuiz:
      "Vilka färger kombineras på kardinalmärken?",
    answer: "Gult och svart.",
    options: [
      "Rött och grönt.",
      "Gult och svart.",
      "Vitt och rött.",
      "Blått och vitt.",
    ],
    correctOptionIndex: 1,
  },
  {
    id: "flytande_sjomarken-2",
    question: "Vilka två färger används på lateralmärken?",
    questionQuiz:
      "Vilka färger har lateralmärken som markerar farledens sidor?",
    answer: "Gröna (styrbord) och röda (babord).",
    options: [
      "Gult och svart.",
      "Blått och vitt.",
      "Gröna (styrbord) och röda (babord).",
      "Rött och vitt.",
    ],
    correctOptionIndex: 2,
  },
  {
    id: "flytande_sjomarken-3",
    question: "Vilket väderstreck ska du passera ett nordmärke på?",
    questionQuiz:
      "På vilken sida om ett nordkardinalmärke ska du passera?",
    answer: "Norr om.",
    options: [
      "Söder om.",
      "Öster om.",
      "Väster om.",
      "Norr om.",
    ],
    correctOptionIndex: 3,
  },

  {
    id: "flytande_sjomarken-4",
    question:
      "Räknat från hindret: Vilket väderstreck finns dessa sjömärken i?",
    questionQuiz:
      "I vilket väderstreck från hindret är detta kardinalmärke placerat?",
    answer: "Öst om hindret.",
    imageKey: "flytande_sjomarken_04_ver02",
    options: [
      "Norr om hindret.",
      "Söder om hindret.",
      "Öst om hindret.",
      "Väster om hindret.",
    ],
    correctOptionIndex: 2,
  },
  {
    id: "flytande_sjomarken-5",
    question:
      "Räknat från hindret: Vilket väderstreck finns dessa sjömärken i?",
    questionQuiz:
      "Vilken riktning från hindret visar detta märke?",
    answer: "Norr om hindret.",
    imageKey: "flytande_sjomarken_05_ver02",
    options: [
      "Norr om hindret.",
      "Söder om hindret.",
      "Öst om hindret.",
      "Väster om hindret.",
    ],
    correctOptionIndex: 0,
  },
  {
    id: "flytande_sjomarken-6",
    question:
      "Räknat från hindret: Vilket väderstreck finns dessa sjömärken i?",
    questionQuiz:
      "Vilket väderstreck från hindret markerar detta sjömärke?",
    answer: "Väster om hindret.",
    imageKey: "flytande_sjomarken_06_ver02",
    options: [
      "Norr om hindret.",
      "Väster om hindret.",
      "Öst om hindret.",
      "Söder om hindret.",
    ],
    correctOptionIndex: 1,
  },
  {
    id: "flytande_sjomarken-7",
    question:
      "Räknat från hindret: Vilket väderstreck finns dessa sjömärken i?",
    questionQuiz:
      "Vilket väderstreck från hindret står dessa märken?",
    answer: "Söder om hindret.",
    imageKey: "flytande_sjomarken_07_ver02",
    options: [
      "Norr om hindret.",
      "Söder om hindret.",
      "Öst om hindret.",
      "Väster om hindret.",
    ],
    correctOptionIndex: 1,
  },

  {
    id: "flytande_sjomarken-8",
    question: "På vilken sida av farleden finns dessa sjömärken?",
    questionQuiz:
      "Vilken sida av farleden markerar dessa lateralmärken?",
    answer: "Babord.",
    imageKey: "flytande_sjomarken_08_ver02",
    options: [
      "Styrbord.",
      "Babord.",
      "Mitt i farleden.",
      "Utanför farleden.",
    ],
    correctOptionIndex: 1,
  },
  {
    id: "flytande_sjomarken-9",
    question: "På vilken sida av farleden finns dessa sjömärken?",
    questionQuiz:
      "Vilken farledssida representeras av dessa märken?",
    answer: "Styrbord.",
    imageKey: "flytande_sjomarken_09_ver02",
    options: [
      "Babord.",
      "Mitt i farleden.",
      "Styrbord.",
      "Utanför farleden.",
    ],
    correctOptionIndex: 2,
  },

  {
    id: "flytande_sjomarken-10",
    question: "Vilken typ av sjömärke är en helt gul boj?",
    questionQuiz:
      "Vad kallas en boj som är helt gul?",
    answer: "Specialmärke.",
    options: [
      "Kardinalmärke.",
      "Lateralmärke.",
      "Specialmärke.",
      "Mittledsmärke.",
    ],
    correctOptionIndex: 2,
  },
  {
    id: "flytande_sjomarken-11",
    question: "Vilken typ av sjömärke är en röd-svart boj?",
    questionQuiz:
      "Vad är en boj med färgerna rött och svart?",
    answer: "Punktmärke.",
    options: [
      "Mittledsmärke.",
      "Specialmärke.",
      "Lateralmärke.",
      "Punktmärke.",
    ],
    correctOptionIndex: 3,
  },
  {
    id: "flytande_sjomarken-12",
    question: "Vilken typ av sjömärke är en vit-rosa (röd) boj?",
    questionQuiz:
      "Vad kallas ett märke som är vitt med rött/rosa vertikalt band?",
    answer: "Mittledsmärke.",
    options: [
      "Punktmärke.",
      "Specialmärke.",
      "Mittledsmärke.",
      "Kardinalmärke.",
    ],
    correctOptionIndex: 2,
  },
];
